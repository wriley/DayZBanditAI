...
for "_i" from 1 to _numTriggers do {
	...
	_attempts = 0;
	while {_trigPos = [(getMarkerPos "DZAI_centerMarker"),300 + random((getMarkerSize "DZAI_centerMarker") select 0),random(360),false,[1,300]] call SHK_pos;(({([_trigPos select 0,_trigPos select 1] distance _x) < (2*((getMarkerSize "DZAI_centerMarker") select 0) - 2*((getMarkerSize "DZAI_centerMarker") select 1)*DZAI_dynOverlap)} count DZAI_dynTriggerArray) > 0)&&(_attempts < 3)} do {
