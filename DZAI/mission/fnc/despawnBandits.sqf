...
if (count _grpArray == 0) exitWith {if (DZAI_debugLevel > 0) then {diag_log "DZAI Extended Debug: Trigger's group array is empty. Exiting despawn script.";};};				//Exit script if the array has spawned no groups.
...
if (triggerActivated _trigger) exitWith {if (DZAI_debugLevel > 0) then {diag_log "DZAI Extended Debug: A player has entered the trigger area. Cancelling despawn script.";};};			//Exit script if trigger has been reactivated since _waitTime seconds has passed.
