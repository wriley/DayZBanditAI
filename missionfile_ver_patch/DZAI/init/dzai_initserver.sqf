...
//Load DZAI variables
call compile preprocessFileLineNumbers "DZAI\init\dzai_config.sqf";

//Load DZAI functions
call compile preprocessFileLineNumbers "DZAI\init\dzai_functions.sqf";

//Load DZAI classname tables
call compile preprocessFileLineNumbers "DZAI\init\world_classname_configs\default\default_classnames.sqf";
...
//_nul = [] call compile preprocessFile "DZAI\scripts\buildWeightedTables.sqf";
...
if (_worldname in ["chernarus","utes","zargabad","fallujah","takistan","tavi","lingor","namalsk","mbg_celle2","oring","panthera2","isladuala","sara","trinity"]) then {
	call compile preprocessFileLineNumbers format ["DZAI\init\world_classname_configs\%1_classnames.sqf",_worldname];
	[] execVM format ["DZAI\init\world_map_configs\world_%1.sqf",_worldname];
} else {
	"DZAI_centerMarker" setMarkerSize [7000, 7000];
	if (DZAI_modName == "epoch") then {
		call compile preprocessFileLineNumbers "DZAI\init\world_classname_configs\epoch\dayz_epoch.sqf";
	};
	DZAI_newMap = true;
...
[] execVM 'DZAI\scripts\DZAI_scheduler.sqf';
